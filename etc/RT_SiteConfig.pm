...
#
#   $ ./configure --enable-layout=Gentoo --with-bin-owner=rt --with-libs-owner=rt --with-libs-group=rt --with-rt-group=rt --with-web-user=apache --with-web-group=apache --with-db-type=Pg --with-db-dba=postgres --with-web-handler=modperl2
#
...
#QuickCalls Tickets
Set($QuickCalls, [{Name => 'Bump up my quota', Queue => 'Quotas', Status => 'open'},
                  {Name => 'Create a new accout', Queue => 'User Account Creation', Status => 'open'}]);
...
Set($CorrespondAddress, 'rt@cs.hmc.edu');

Set($CommentAddress, 'rt@cs.hmc.edu');
...
Set($LogToFile, undef);
Set($LogDir, q{/var/www/localhost/rt-4.2.11/var/log});
...
FOR THIS SYSTEM TO WORK, MAKE SURE THIS VALUE IS 'qmail' -LJT
...
Set($SetOutgoingMailFrom, "ljthomas@cs.hmc.edu");
...
Set($SendmailArguments, "-oi -ODeliveryyMode=q");
...
Set($SendmailPath, "/usr/bin/nullmailer-smtpd");
...
Set(
    $HomepageComponents,
    [
        qw(QuickCreate Quicksearch MyAdminQueues MySupportQueues MyReminders RefreshHomepage Dashboards SavedSearches FindUser QuickCalls) # loc_qw
    ]
);
...
Set($DefaultSearchResultOrderBy, 'Priority');
...
fields, as well as  fields, which are specified as "CF.1234" or
"CF.Name"
...
Set($DisplayTicketAfterQuickCreate, undef);
...
=important note to avoid cross-site request forgery

makes sure to whitelist the host machine!
=cut
Set(@ReferrerWhitelist, qw(134.173.43.164:443 127.0.0.1:80));
...
Set( %SMIME,
    Enable => 0,
    OpenSSL => 'openssl',
    Keyring => q{/var/www/localhost/rt-4.2.11/var/data/smime},
    CAPath => undef,
...
Set(%GnuPGOptions,
    homedir => q{/var/www/localhost/rt-4.2.11/var/data/gpg},
